...
class progress_cards:
    def __init__(self):
        self.progress = ["Monopoly"] * 2 + ["Roads"] * 2 + ["Invention"] * 2 + ["Victory points"] * 5 + ["Knights"] * 14 
        random.shuffle(self.progress)

    def takes_card_p(self):
        return self.progress.pop() if self.progress else None 


class materials_cards:
    def __init__(self):
        self.materials = ["Wood"] * 19 + ["Clay"] * 19 + ["Rock"] * 19 + ["Sheep"] * 19 + ["Wheat"] * 19
        random.shuffle(self.materials)

    def takes_card_m(self):
        return self.materials.pop() if self.materials else None
